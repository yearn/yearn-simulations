...
@schedule_script("@yfitestchannel", minute="0", hour="16")
def main():
    load_dotenv()
    env = os.environ.get("ENV")  # Set environment
    test_group = os.getenv("TELEGRAM_CHAT_ID_TEST_GROUP")
    chat_id = os.environ.get("TELEGRAM_CHAT_ID", os.environ.get("TELEGRAM_CHAT_ID_BRIBE"))
    # chat_id = test_group
    ...
    gauge_controller = interface.IGaugeController(
    ...
    for i in range(0, num_gauges):
        ...
        if len(rewards) > 0:
            ...
            for i, r in enumerate(rewards):
                ...
                total_tokens = (
                    total_tokens
                    * gauge_controller.points_weight(g, period).dict()["slope"]
                    / 1e18
                )
    ...
    message = (
        message
        + "Next available vote date: "
        + datetime.utcfromtimestamp(next_vote_available).strftime("%Y-%m-%d %H:%M:%S")
    )
    print(message)
    if env == "PROD":
        bot.send_message(
